...
Usage:
    ...
    python setup.py --tool opencode  # Generate for OpenCode
...
ALL_TOOLS = ["claude", "gemini", "codex", "antigravity", "opencode"]
...
def _opencode_description(skill):
    """OpenCode description field is limited to 1024 characters."""
    return _antigravity_description(skill)[:1024]
...
def _write_opencode_skill(skill, dest_dir):
    name = skill["name"]
    skill_id = f"fk-{name}"
    folder = dest_dir / skill_id
    folder.mkdir(parents=True, exist_ok=True)
    desc = _yaml_scalar(_opencode_description(skill))
    content = (
        f"---\n"
        f"name: {skill_id}\n"
        f"description: {desc}\n"
...
def _write_opencode_command(skill, dest_dir):
    name = skill["name"]
    dest_dir.mkdir(parents=True, exist_ok=True)
    dest = dest_dir / f"fk-{name}.md"
    desc = _yaml_scalar(_opencode_description(skill))
...
def generate_opencode(skills):
    """Create OpenCode skills and slash commands.

    Layout (https://opencode.ai/docs/skills, /docs/commands):
      .opencode/skills/fk-<name>/SKILL.md  — Agent Skills (progressive disclosure)
      .opencode/commands/fk-<name>.md      — slash commands /fk-<name>
      AGENTS.md                            — project rules OpenCode loads every session
    """
    for skill in skills:
        _write_opencode_skill(skill, OPENCODE_SKILLS_DIR)
        _write_opencode_command(skill, OPENCODE_COMMANDS_DIR)

    generate_codex(skills)
    print(
        f"Generated {len(skills)} OpenCode skills in .opencode/skills/ "
        f"and commands in .opencode/commands/"
    )
    return len(skills)
...
def do_interactive(skills):
    ...
    print("  [5] OpenCode          (.opencode/skills + commands)")
...
def do_clean():
    ...
    if OPENCODE_SKILLS_DIR.exists():
        for folder in OPENCODE_SKILLS_DIR.glob("fk-*"):
            if folder.is_dir():
                shutil.rmtree(folder)
                removed.append(str(folder))
    if OPENCODE_COMMANDS_DIR.exists():
        for f in OPENCODE_COMMANDS_DIR.glob("fk-*.md"):
            f.unlink()
            removed.append(str(f))
...
def main():
    ...
    parser.add_argument(
        "--tool",
        choices=["claude", "gemini", "codex", "antigravity", "opencode", "all"],
