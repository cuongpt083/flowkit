...
if ($chars.Count -eq 0) { throw "No entities on project $pid" }
...
if ($reqs.Count -eq 0) {
    ...
    $st = Wait-FkBatchStatus -ProjectId $pid -Type $type -IntervalSeconds $PollSeconds
...
Write-Host ("All references ready. Run /fk-gen-images {0} <VID> to generate scene images." -f $pid)
