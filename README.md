...
Skills handle all the API calls, polling, and verification automatically. Use with Claude Code or OpenCode (`/fk-command`) or follow the recipe in `skills/*.md` for any AI agent.
...
Ready-to-use workflow recipes in `skills/` (also available as `/slash-commands` in Claude Code and OpenCode):
...
```bash
python setup.py --tool opencode   # or claude / gemini / codex / all
```
...
AGENTS.md                # AI agent instructions (Codex CLI / OpenCode)
